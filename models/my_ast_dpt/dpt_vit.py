...
import timm
from ast_backbone import ASTModel
...
def forward_flex(self, x):
    ...
    if getattr(self.v, "dist_token", None) is not None:
        cls_tokens = self.v.cls_token.expand(
            B, -1, -1
...
def _resize_pos_embed(self, posemb, gs_h, gs_w):
    ...
    # old grid size
    num_old_patches = posemb_grid.shape[0]
    gs_h_old = self.p_fshape  # pretraining frequency dimension
    gs_w_old = self.p_tshape  # pretraining time dimension

    posemb_grid = posemb_grid.reshape(1, gs_h_old, gs_w_old, -1).permute(0, 3, 1, 2)
...
def _make_ast_b16_backbone(
    ...
    pretrained.model.v.blocks[hooks[0]].register_forward_hook(get_activation("1"))
    pretrained.model.v.blocks[hooks[1]].register_forward_hook(get_activation("2"))
    pretrained.model.v.blocks[hooks[2]].register_forward_hook(get_activation("3"))
    pretrained.model.v.blocks[hooks[3]].register_forward_hook(get_activation("4"))
