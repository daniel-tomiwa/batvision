from utils.dataloader import BatvisionV2DatasetTrain, BatvisionV2DatasetTest
...
def main(cfg: DictConfig):
    ...
    if cfg.dataset.name == 'batvisionv2':
        ...
        if cfg.mode.mode == 'train':
            train_set = BatvisionV2DatasetTrain(cfg, cfg.dataset.annotation_file_train, subset_number=cfg.dataset.subset_number) 
            ...
            if cfg.mode.validation:
                val_set = BatvisionV2DatasetTrain(cfg, cfg.dataset.annotation_file_val, subset_number=cfg.dataset.subset_number) 
            ...
            if cfg.mode.eval_on == 'val':
                eval_set = BatvisionV2DatasetTest(cfg, cfg.dataset.annotation_file_val, subset_number=cfg.dataset.subset_number) 
            else:
                eval_set = BatvisionV2DatasetTest(cfg, cfg.dataset.annotation_file_test, subset_number=cfg.dataset.subset_number) 
    ...
    summary(model.to(device), torch.zeros((2, 2, 256, 256)).to(device))
    # os._exit(0)
